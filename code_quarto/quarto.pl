...
jeu(Plateau,PiecesDisponibles,PlacesOccupees,JoueurCourant,JoueurPassif):-
    ...
    write("Voici toutes les pièces disponibles "),afficherLigne(PiecesDisponibles),nl,nl,
    %on récupere ce qu'il a ecrit
    choixPion(Pion,JoueurPassif),entrerPion(PiecesDisponibles,Pion,PionFinal,JoueurPassif),
    write(JoueurPassif),write(" a choisi la pièce suivante : "),writeln(PionFinal),nl,
